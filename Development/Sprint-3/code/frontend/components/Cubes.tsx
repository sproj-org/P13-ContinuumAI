...
const Cubes: React.FC<CubesProps> = ({
  ...
  return (
    <div className="absolute inset-0 w-full h-full flex items-center justify-center overflow-hidden" style={wrapperStyle}>
      <div ref={sceneRef} className="grid w-full h-full overflow-hidden" style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => (
            <div
              key={`${r}-${c}`}
