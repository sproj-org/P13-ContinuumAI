...
import { mockApiCall, type ApiResponse, type PlotlyChart as PlotlyChartType } from '@/lib/mockApiResponses';
...
export function ChatInterface({ isSidebarOpen, onToggleSidebar }: ChatInterfaceProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      // Call mock API (this will be replaced with real API call later)
      const response = await mockApiCall(userQuery);
      
      if (response.status === 'success') {
        // Add AI message with optional text and chart data
        addMessage(activeChat.id, {
          content: response.text || '', // Include text explanation if available
          sender: 'ai',
